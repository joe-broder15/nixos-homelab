...
- `configurations/thinkpad/configuration.nix` contains the desktop configuration, including a CIFS mount for `//synology.local/Library1`.
- `configurations/thinkpad/hosts.nix` adds a static `networking.hosts` entry resolving `synology.local` to `192.168.1.99` so the CIFS mount works without DNS.
- Hardware configuration is read from `/etc/nixos/hardware-configuration.nix` on the host (not versioned in this repo).
